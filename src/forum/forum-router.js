...
forumRouter.get("/catagories", (req, res, next) => {
...
forumRouter.get("/messageBoards", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.get("/newestPosts", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.get("/numOfThreads/:board_id", (req, res, next) => {
  const db = req.app.get("db");
  const { board_id } = req.params;
  ForumService.getNumOfThreads(db, board_id)
    .then(numOfThreads => {
      if (!numOfThreads) {
        return res.status(404).json({ error: "Message Board does not exist" });
      }
...
forumRouter.get("/messageboards/get-board/:id", (req, res, next) => {
  const db = req.app.get("db");
  var { id } = req.params;
  id = parseInt(id, 10);
...
forumRouter.get("/messageboards/get-board-name/:id", (req, res, next) => {
  const db = req.app.get("db");
  var { id } = req.params;
  id = parseInt(id, 10);
...
forumRouter.get(
  "/messageboards/get-board-posts/:board_id",
  (req, res, next) => {
    const db = req.app.get("db");
...
forumRouter.get("/messageboards/get-post-by-id/:id", (req, res, next) => {
  const db = req.app.get("db");
  var { id } = req.params;
  id = parseInt(id, 10);
...
forumRouter.get("/messageboards/:messageboard_section", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.get("/posts", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.get("/get-posts-for/:board_id", (req, res, next) => {
  const db = req.app.get("db");
  const { board_id } = req.params;
...
forumRouter.get("/search/posts/:term", (req, res, next) => {
  const db = req.app.get("db");
  var { term } = req.params;
  let posts = {};
...
forumRouter.get("/search/posts/:board_id/:term", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.get("/likesTracker/:user_id/:post_id", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.get("/sort/:column/:sortType", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.post("/addPost", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.patch("/edit", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.patch("/post/addLike/:post_id", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.patch("/post/minusLike/:post_id", (req, res, next) => {
  const db = req.app.get("db");
  const { post_id } = req.params;
  if (!post_id) {
    return res
  ...
  ForumService.minusLike(db, post_id)
    .then(row => {
      if (!row) {
        return res.status(401).json({ error: "Could not minus like to post." });
      }
      return res.status(201).json(row);
    })
    .catch(err => {
...
forumRouter.post("/post/addToTracker", (req, res, next) => {
  const db = req.app.get("db");
...
forumRouter.delete("/post/deleteFromTracker", (req, res, next) => {
  const db = req.app.get("db");
  const { post_id, user_id } = req.body;
...
forumRouter.delete("/posts/:post_id", (req, res, next) => {
  const db = req.app.get("db");
